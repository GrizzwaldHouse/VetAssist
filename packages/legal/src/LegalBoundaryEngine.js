...
function rewriteDirective(text) {
    let rewritten = text;
...
function rewriteEligibility(text) {
    let rewritten = text;
...
function rewriteOutcomeGuarantees(text) {
    let rewritten = text;
    for (const pattern of OUTCOME_GUARANTEE_PATTERNS) {
        rewritten = rewritten.replace(pattern, 'veterans with similar documentation have received');
    }
    return rewritten;
}
function detectViolations(text) {
    ...
    if (DIRECTIVE_PATTERNS.some((p) => p.test(text)))
        types.push('directive_language');
    if (ELIGIBILITY_PATTERNS.some((p) => p.test(text)))
        types.push('eligibility_statement');
    if (OUTCOME_GUARANTEE_PATTERNS.some((p) => p.test(text)))
        types.push('outcome_guarantee');
...
async function enforceAndEmit(response) {
...
//# sourceMappingURL=LegalBoundaryEngine.js.map