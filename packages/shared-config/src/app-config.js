...
function loadAppConfig() {
    ...
    if (!apiUrl)
        throw new Error('[AppConfig] VETASSIST_API_URL is required');
    if (!authDomain)
        throw new Error('[AppConfig] AUTH0_DOMAIN is required');
    if (!authClientId)
        throw new Error('[AppConfig] AUTH0_CLIENT_ID is required');
    if (!chromaUrl)
        throw new Error('[AppConfig] CHROMA_URL is required');
    return {
        apiUrl,
        authDomain,
        authClientId,
